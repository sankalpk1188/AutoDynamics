@php
    ...
    $configJson = json_encode($showcaseData, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE);
    $svgBase = asset('assets/svg/');
    if (!str_ends_with($svgBase, '/')) {
        $svgBase .= '/';
    }
    $firstView = ($showcaseData['svgViews'][0] ?? 'hospital-bed');
    $firstHeroImg = $vehicleImages[$firstView] ?? asset('assets/images/industrial/hospital-bed-hero.png');
@endphp

<main class="as-page" id="industrial-showcase"
      data-product-showcase
      data-config="{{ $configJson }}"
      data-svg-base="{{ $svgBase }}">

    <section class="as-hero" aria-label="Industrial products introduction">
                ...
                <div class="as-content">
                    <div class="as-visual-stage" data-as-stage>
                        ...
                        <div class="as-split-visual" data-as-split-visual>
                            ...
                            <div class="as-vehicle-bg-layer">
                                <img class="as-vehicle-bg"
                                     data-as-vehicle-bg
                                     src="{{ $firstHeroImg }}"
                                     alt="Hospital bed structural view"
                                     loading="eager">
                        ...
                        <aside class="as-part-callout is-visible" data-as-part-name-bar aria-live="polite">
                            <span class="as-part-name-category" data-as-category>{{ $categories[0]['label'] ?? 'Hospital Bed' }}</span>
